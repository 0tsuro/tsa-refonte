...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
    $form_errors[] = 'Security error: invalid token.';
  } else {
    // Sanitize data
    ...
    // Validation
    if (empty($prenom)) $form_errors[] = 'First name is required.';
    if (empty($nom)) $form_errors[] = 'Last name is required.';
    if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) $form_errors[] = 'Invalid email.';
    if (empty($telephone)) $form_errors[] = 'Phone number is required.';
    if (empty($message)) $form_errors[] = 'Message is required.';
    if (!$rgpd) $form_errors[] = 'Please accept the GDPR policy.';
    
    // Validation du captcha et honeypot
    if (empty($recaptcha_token)) {
      $form_errors[] = 'Please validate the reCAPTCHA.';
    } elseif (!validate_recaptcha($recaptcha_token)) {
      $form_errors[] = 'reCAPTCHA validation failed. Please try again.';
    }
    ...
    if (empty($form_errors)) {
      ...
      $subject = "New message from $prenom $nom - $objet";
      $email_body = create_email_body($email_data);
      
      // Send email via SMTP (FROM will automatically be the authenticated SMTP account)
      if (send_email_smtp(CONTACT_EMAIL, $subject, $email_body)) {
        ...
        $form_errors[] = 'Error sending the message. Please try again.';
      }
    }
    $form_submitted = true;
  }
...
// Generate the captcha if it doesn't exist
        ...
        <div class="contact-inner">
          ...
          <div class="contact-left">
            ...
            <div class="contact-socials">
              <h3 class="contact-socials__title">Follow Us</h3>
          ...
          <div class="contact-right">
            <?php if ($form_submitted && $form_success): ?>
              <div class="contact-form__success">
                ...
                <p>We have received your request. Our team will respond to you as soon as possible.</p>
              </div>
            <?php endif; ?>
            ...
            <form class="contact-form" method="POST" action="">
              ...
              <div class="contact-form__row">
                <div class="contact-form__group">
                  <label for="prenom" class="contact-form__label">First Name*</label>
                  <input type="text" id="prenom" name="prenom" class="contact-form__input" required value="<?php echo htmlspecialchars($prenom ?? ''); ?>">
                </div>

                <div class="contact-form__group">
                  <label for="nom" class="contact-form__label">Last Name*</label>
              ...
              <div class="contact-form__row">
                ...
                <div class="contact-form__group">
                  <label for="telephone" class="contact-form__label">Phone*</label>
              ...
              <div class="contact-form__group contact-form__group--full">
                <label for="objet" class="contact-form__label">Subject</label>
                <select id="objet" name="objet" class="contact-form__select">
                  <option value="">Select a subject</option>
                  <option value="commercial" <?php echo ($objet === 'commercial') ? 'selected' : ''; ?>>Commercial request</option>
                  <option value="partenariat" <?php echo ($objet === 'partenariat') ? 'selected' : ''; ?>>Partnership</option>
                  <option value="reclamation" <?php echo ($objet === 'reclamation') ? 'selected' : ''; ?>>Complaint</option>
                  <option value="autre" <?php echo ($objet === 'autre') ? 'selected' : ''; ?>>Other</option>
                </select>
              </div>
              ...
              <div class="contact-form__checkbox">
                <p class="contact-form__required-note">* required fields</p>
                <div class="contact-form__checkbox-wrapper">
                  ...
                  <label for="rgpd" class="contact-form__checkbox-label">I accept the privacy policy and the processing of my personal data</label>
                </div>
              </div>
